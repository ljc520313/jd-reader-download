export function PC1() {
    this.hexCharsToBytes = this.hexCharsToBytes || (function () {
        var C = new Array(256);
        var D;
        for (D = 0; D < 10; ++D) {
            C[48 + D] = D
...
PC1.prototype = {
    ...
    stringToBytes: function (G: string) {
        var I, F, J: any[] = [];
    ...
    utf16ToBytes: function (G: string) {
        var I, F, J = new Array(G.length << 1);
        for (var H = 0; H < G.length; H++) {
            I = G.charCodeAt(H);
            J[H * 2] = I >> 8;
            J[H * 2 + 1] = I & 255
        ...
        return F
    }, bytesToHex: function (F: string | any[]) {
